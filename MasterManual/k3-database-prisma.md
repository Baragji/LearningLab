...
* **Løsningsskabeloner (Eksempler):**
    ...
    * **Rettelse af `NOT NULL` migration (Eksempel for `passwordHash`):**
        ...
        * **Metode 2: Gør kolonnen nullbar først (foretrukket hvis data skal bevares og opdateres):**
            ...
            ```
            *(Prisma forsøger selv at håndtere dette ved at gøre kolonnen nullbar, tilføje data, og så gøre den NOT NULL, men det kan fejle. Manuel SQL giver mere kontrol).*
...
* **Løsningsskabeloner (Eksempler):**
    ...
    * **Brug `prisma migrate resolve` (med stor forsigtighed):**
        * Hvis du er *sikker* på, at migrationen *er* anvendt korrekt manuelt, eller at den *skal* springes over:
            ```bash
...
* **Løsningsskabeloner (Eksempler):**
    ...
    * **Opdater `yarn seed` script i rod `package.json`:**
        ...
        ```
        *(Sørg for at `ts-node` er en devDependency i `apps/api` eller globalt, hvis du bruger `ts-node prisma/seed.ts` direkte).*
...
* **Løsningsskabeloner (Eksempler):**
    ...
    * **Juster `Dockerfile.api` for at sikre Prisma Client i produktionsimaget:**
        ...
        **Metode B: Kopier den genererede client eksplicit (hvis `generate` er langsom):**
        ...
        CMD ["node", "apps/api/dist/main.js"]
        ```
        *(Metode A er ofte mere robust, da den sikrer, at clienten genereres i det præcise runtime-miljø).*
