...
- **Løsningsskabeloner (Eksempler):**
  ...
  - **Korrekt brug af `useEffect` for DOM manipulation (sjældent nødvendigt):**
    - DOM manipulationer bør primært ske gennem Reacts state og props. Hvis direkte manipulation er uundgåelig, sørg for det sker _efter_ første mount i `useEffect`.
...
- **Løsningsskabeloner (Eksempler):**
  ...
  - **Installation af `sharp` dependencies i `Dockerfile.web` (hvis Next.js's default optimizer bruges):**
    ...
    _Vigtig note om `sharp` i Docker:_ Med nyere Next.js versioner (12+) er `sharp` ikke altid et hårdt krav for billedoptimering, da Next.js har indbygget optimering via SWC. Hvis du _ikke_ ser `sharp`-relaterede fejl, er det sandsynligvis ikke nødvendigt at installere `vips-dev` osv. i din Alpine-container. Hvis du _gør_, så er ovenstående linjer relevante. Vercel og lignende platforme håndterer typisk billedoptimering for dig.
