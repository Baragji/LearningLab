...
* **Løsningsskabeloner (Eksempler):**
    * **Opdater `package-scripts.js` til at bruge `yarn` (anbefalet):**
        ...
        ```
        *Bemærk: For at `yarn workspace [navn] build` skal virke i `out` mappen, skal `out/package.json` (genereret af `turbo prune`) indeholde workspaces-definitionen, eller også skal kommandoen køres fra `out`-mappens rod med `yarn build` hvis scriptet er defineret i `out/package.json`'s rod-scripts.*
        *En mere robust tilgang i `package-scripts.js` for CI build i `out` mappen:*
        ```javascript
        // package-scripts.js
            build: {
              ci: {
        ...
        ```
        *Sørg for, at `build`-scriptet i `apps/web/package.json` og `apps/api/package.json` er korrekt:*
        ```json
