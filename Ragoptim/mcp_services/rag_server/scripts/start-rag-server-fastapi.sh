...
# Installer FastAPI og Uvicorn hvis de ikke allerede er installeret
pip install fastapi uvicorn

# Ryd Python cache for at sikre friske imports
find "${PROJECT_ROOT}/mcp_services/rag_server" -name "__pycache__" -type d -exec rm -rf {} + 2>/dev/null || true
...
if [ ! -d "${PROJECT_ROOT}/chroma_db" ] || [ -z "$(ls -A ${PROJECT_ROOT}/chroma_db 2>/dev/null)" ]; then
    echo "ChromaDB directory is empty or doesn't exist. Running indexing script..."
    python "${PROJECT_ROOT}/mcp_services/rag_server/index_code_chunks.py"
fi
...
uvicorn mcp_services.rag_server.vector_search_server_fastapi:app --host 0.0.0.0 --port 5004 --workers 4 > "${PROJECT_ROOT}/logs/rag_server.log" 2>&1 &

# Gem PID for nem reference
echo $! > "${PROJECT_ROOT}/logs/rag_server.pid"
echo "✅ RAG-server startet med PID $(cat ${PROJECT_ROOT}/logs/rag_server.pid)"
...
echo "🛑 Stop serveren med: ${PROJECT_ROOT}/mcp_services/rag_server/scripts/stop-rag-server.sh"