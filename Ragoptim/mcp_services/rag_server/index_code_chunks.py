...
# Gør det muligt at importere rank_chunks og semantic_chunking
...
def read_and_chunk_file(file_path: str):
    """
    Læser indholdet af file_path, opdeler i semantiske chunks,
    ...
    # Prøv først med AST-baseret chunking for Python-filer
    chunks_with_meta = []
    if file_ext == ".py":
        chunks_with_meta = smart_chunk_file(content, file_path)
    
    # Hvis ingen chunks blev fundet med AST, eller for andre filtyper, brug semantisk chunking
    if not chunks_with_meta:
        # Brug semantisk chunking
        semantic_chunks = semantic_chunk(content, min_size=300, max_size=600, model=embedding_model)
        
        for i, chunk_text in enumerate(semantic_chunks):
            meta = {
                "type": "semantic_chunk",
                "start_line": i,  # Approksimeret startlinje
                "end_line": i + chunk_text.count('\n'),  # Approksimeret slutlinje
            }
