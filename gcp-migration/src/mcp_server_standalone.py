...
async def startup_event():
    ...
    global rag_engine
    try:
        # Try to initialize RAG engine, but don't fail if it's not available
        try:
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "services": {
            "rag_engine": rag_engine is not None and hasattr(rag_engine, 'is_ready') and rag_engine.is_ready(),
            "mcp_server": True
        }
    }
...
async def handle_tool_call(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        if rag_engine and hasattr(rag_engine, 'is_ready') and rag_engine.is_ready():
            # Use real RAG engine if available
            ...
            if tool_name == "analyze_code":
                result = f"Code analysis for: {arguments.get('code', '')[:100]}...\n\nRAG engine not available. This is a placeholder response."
            elif tool_name == "search_codebase":
...
async def handle_resource_read(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if uri == "codebase://":
        ...
        if rag_engine and hasattr(rag_engine, 'get_codebase_stats'):
