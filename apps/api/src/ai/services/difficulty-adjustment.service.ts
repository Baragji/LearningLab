...
export interface DifficultyAdjustmentRule {
  id: string;
  name: string;
  condition: (performance: UserPerformanceData[], currentLevel: DifficultyLevel) => boolean;
  adjustment: {
...
export class DifficultyAdjustmentService {
  ...
  private readonly adjustmentRules: DifficultyAdjustmentRule[] = [
    {
      ...
      condition: (performance, currentLevel) => {
        ...
        const avgAccuracy = recentSessions.reduce((sum, p) => 
          sum + (p.correctAnswers / p.totalQuestions), 0) / recentSessions.length;
        return avgAccuracy > 0.85 && recentSessions.length >= 3;
      },
      adjustment: {
        direction: 'increase',
        magnitude: 1.2,
        reason: 'Konsistent høj performance indikerer behov for større udfordring',
      },
      priority: 8,
    },
    {
      ...
      adjustment: {
        direction: 'decrease',
        magnitude: 0.8,
        reason: 'Lav performance kræver lettere spørgsmål for at opbygge selvtillid',
      },
      priority: 9,
    },
    {
      ...
      condition: (performance, currentLevel) => {
        ...
        const recentAvg = recent.reduce((sum, p) => 
          sum + (p.correctAnswers / p.totalQuestions), 0) / recent.length;
        const olderAvg = older.reduce((sum, p) => 
          sum + (p.correctAnswers / p.totalQuestions), 0) / older.length;
    ...
    },
    {
      ...
      condition: (performance, currentLevel) => {
        ...
        const accuracies = recent.map(p => p.correctAnswers / p.totalQuestions);
    ...
    },
    {
      ...
      condition: (performance, currentLevel) => {
        ...
        const avgAccuracy = recentSessions.reduce((sum, p) => 
          sum + (p.correctAnswers / p.totalQuestions), 0) / recentSessions.length;
        const hasLongSessions = recentSessions.some(p => p.totalStudyTime > 60);
    ...
    },
    {
      ...
      condition: (performance, currentLevel) => {
        const recentSessions = performance.slice(-3);
        const avgTimePerQuestion = recentSessions.reduce((sum, p) => 
          sum + (p.totalStudyTime / p.totalQuestions), 0) / recentSessions.length;
        const avgAccuracy = recentSessions.reduce((sum, p) => 
          sum + (p.correctAnswers / p.totalQuestions), 0) / recentSessions.length;
      ...
      adjustment: {
        direction: 'increase',
        magnitude: 1.4,
        reason: 'Hurtig og nøjagtig performance tillader mere komplekse opgaver',
      },
      priority: 7,
    },
  ];
  ...
  async adjustDifficultyLevel(
    ...
    try {
      this.logger.log(`Adjusting difficulty for user ${userId}, current level: ${currentLevel.level}`);

      // Apply adjustment rules
      const applicableRules = this.findApplicableRules(performanceHistory, currentLevel);
      const adjustment = this.calculateAdjustment(applicableRules, currentLevel);
      
      // Calculate new difficulty level
      const newLevel = this.calculateNewLevel(currentLevel, adjustment);
      
      // Calculate performance metrics
      const performanceMetrics = this.calculatePerformanceMetrics(performanceHistory);
      ...
      const nextReviewDate = this.calculateNextReviewDate(newLevel, performanceMetrics);
      ...
      this.logger.log(`Difficulty adjusted for user ${userId}: ${currentLevel.level} -> ${newLevel.level}`);
      return result;
    } catch (error) {
  ...
  async performRealTimeAdjustment(
    ...
    try {
      ...
      const nextQuestionRecommendation = this.generateNextQuestionRecommendation(
        adjustedDifficulty,
        questionMetadata,
        sessionContext,
      );
  ...
  async calibrateQuestionDifficulty(
    ...
    try {
      ...
      const levelSuccessRates = new Map<string, { correct: number; total: number }>();
  ...
  private calculateNewLevel(
    ...
    if (adjustment.direction === 'increase') {
      newNumericValue = Math.min(10, currentLevel.numericValue * adjustment.magnitude);
    } else {
      newNumericValue = Math.max(1, currentLevel.numericValue * adjustment.magnitude);
    }
  ...
  private calculatePerformanceMetrics(
    ...
    const accuracies = recentSessions.map(p => p.correctAnswers / p.totalQuestions);
    const accuracyTrend = this.calculateTrend(accuracies);
    
    // Calculate speed trend (questions per minute)
    const speeds = recentSessions.map(p => p.totalQuestions / (p.totalStudyTime / 60));
    ...
    const avgStudyTime = recentSessions.reduce((sum, p) => sum + p.totalStudyTime, 0) / recentSessions.length;
...
Seneste performance:
${performanceHistory.slice(-3).map(p => 
...
`;
    ...
    try {
      const response = await this.aiProviderService.generateChatCompletion([
        {
          role: 'system',
          content: 'Du er en AI læringskonsulent der genererer specifikke anbefalinger for sværhedsgrads justeringer. Svar kun med valid JSON på dansk.',
        },
        {
          role: 'user',
          content: prompt,
      ...
      this.logger.warn('Failed to generate AI recommendations, using fallback', error);
      ...
      if (newLevel.numericValue > currentLevel.numericValue) {
        ...
        recommendations.push('Øv dig med lettere opgaver for at opbygge selvtillid');
      } else {
  ...
  private calculateAdaptationSettings(
    ...
    const avgAccuracy = recentPerformance.reduce((sum, p) => 
      sum + (p.correctAnswers / p.totalQuestions), 0) / recentPerformance.length;
    
    // More aggressive adaptation for consistent performers
    const aggressiveness = avgAccuracy > 0.8 ? 0.8 : 
                          avgAccuracy > 0.6 ? 0.6 : 0.4;
    
    // Longer stability for struggling learners
    const stabilityThreshold = avgAccuracy < 0.5 ? 8 : 
                              avgAccuracy < 0.7 ? 5 : 3;
  ...
  private calculateRealTimeFactors(
    ...
    const userPerformance = userResponse.isCorrect ? 
      (userResponse.confidence || 0.8) : 0.3;
    
    // Time factor (0.5 = too slow, 1.0 = optimal, 1.2 = very fast)
    const expectedTime = questionMetadata.timeEstimate;
    const timeFactor = expectedTime > 0 ? 
      Math.min(1.2, expectedTime / userResponse.timeSpent) : 1.0;
    ...
    const sessionFactor = sessionContext.currentAccuracy > 0.8 ? 1.1 : 
                         sessionContext.currentAccuracy < 0.5 ? 0.8 : 1.0;
  ...
  private generateNextQuestionRecommendation(
    ...
    const preferredTopics = sessionContext.currentAccuracy > 0.7 ? 
      questionMetadata.topics : [];
    
    // Avoid topics if struggling
    const avoidTopics = sessionContext.currentAccuracy < 0.5 ? 
      questionMetadata.topics : [];
  ...
  private async getAICalibration(
    ...
    try {
      const successRateData = Array.from(levelSuccessRates.entries())
        .map(([level, stats]) => `${level}: ${(stats.correct / stats.total * 100).toFixed(1)}%`)
        .join(', ');
...
`;
      ...
      const response = await this.aiProviderService.generateChatCompletion([
        {
          role: 'system',
          content: 'Du er en AI der kalibrerer spørgsmåls sværhedsgrad. Svar kun med valid JSON.',
        },
        {
          role: 'user',
          content: prompt,
      ...
      this.logger.warn('Failed to get AI calibration, using statistical fallback', error);
      ...
      const calibratedDifficulty = successRate > 0.8 ? 3 :
                                  successRate > 0.6 ? 5 :
                                  successRate > 0.4 ? 7 : 9;
