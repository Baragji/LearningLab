...
import { AdaptiveLearningService, UserPerformanceData } from './adaptive-learning.service';
...
export class LearningAnalyticsService {
  ...
  async generateDashboardData(
    ...
    try {
      ...
      this.logger.log(`Successfully generated dashboard data for user ${userId}`);
  ...
  async calculateLearningMetrics(
    ...
    try {
      ...
      const filteredData = this.filterDataByTimeframe(performanceHistory, timeframe);
      ...
      const totalStudyTime = filteredData.reduce((sum, data) => sum + data.totalStudyTime, 0);
      const sessionsCompleted = filteredData.length;
      const averageSessionDuration = totalStudyTime / sessionsCompleted;
      const questionsAnswered = filteredData.reduce((sum, data) => sum + data.totalQuestions, 0);
      const correctAnswers = filteredData.reduce((sum, data) => sum + data.correctAnswers, 0);
      ...
      // Calculate streaks
      const { currentStreak, longestStreak } = this.calculateStreaks(filteredData);

      // Analyze concepts
      const conceptAnalysis = this.analyzeConceptProgress(filteredData);

      // Calculate difficulty progression
      const difficultyProgression = this.calculateDifficultyProgression(filteredData);
    ...
    try {
      ...
      performanceHistory.forEach(session => {
        ...
        allConcepts.forEach(concept => {
          ...
          conceptData.lastPracticed = new Date(Math.max(
      ...
      for (const [conceptName, data] of conceptMap.entries()) {
        ...
        const masteryLevel = this.calculateMasteryLevel(accuracy, data.timeSpent, data.sessions.length);
  ...
  async analyzeLearningPatterns(
    ...
    try {
      ...
      const optimalStudyTimes = this.identifyOptimalStudyTimes(performanceHistory);
      
      // Calculate preferred session length
      const sessionLengths = performanceHistory.map(p => p.totalStudyTime);
      const preferredSessionLength = this.calculateOptimalSessionLength(sessionLengths);
      
      // Analyze learning style confidence
      const learningStyleConfidence = this.analyzeLearningStyleConfidence(performanceHistory);
      
      // Identify motivation factors and challenges
      const motivationFactors = this.identifyMotivationFactors(performanceHistory);
  ...
  async generateProgressPrediction(
    ...
    try {
      ...
      const currentTrajectory = this.assessCurrentTrajectory(metrics, performanceHistory);
...
`;
    ...
    try {
      const response = await this.aiProviderService.generateChatCompletion([
        {
          role: 'system',
          content: 'Du er en AI læringskonsulent der genererer personaliserede indsigter og anbefalinger. Svar kun med valid JSON på dansk.',
        },
        {
          role: 'user',
          content: prompt,
  ...
  private async generateVisualizationData(
    ...
    try {
      ...
      const studyTimeDistribution = this.generateStudyTimeChart(performanceHistory);
      
      // Concept mastery radar
      const conceptMasteryRadar = this.generateConceptMasteryRadar(conceptMastery);
      
      // Difficulty progression
      const difficultyProgression = this.generateDifficultyProgressionChart(metrics.difficultyProgression);
      
      // Engagement heatmap
      const engagementHeatmap = this.generateEngagementHeatmap(performanceHistory);
  ...
  private filterDataByTimeframe(
    ...
    return data.filter(item => 
      item.lastActivity >= timeframe.start && item.lastActivity <= timeframe.end
    );
  }
  ...
  private calculateStreaks(data: UserPerformanceData[]): { currentStreak: number; longestStreak: number } {
    // Simplified streak calculation
    const sortedData = data.sort((a, b) => a.lastActivity.getTime() - b.lastActivity.getTime());
    ...
    for (let i = 0; i < sortedData.length; i++) {
      const accuracy = (sortedData[i].correctAnswers / sortedData[i].totalQuestions) * 100;
  ...
  private calculateDifficultyProgression(data: UserPerformanceData[]): DifficultyProgressionData[] {
    return data.map(session => ({
      date: session.lastActivity,
      difficultyLevel: session.difficultyLevel,
      accuracy: (session.correctAnswers / session.totalQuestions) * 100,
      confidence: Math.min(100, (session.correctAnswers / session.totalQuestions) * 100 + 
        (session.streakDays * 2)), // Simple confidence calculation
    }));
  }
  ...
  private calculateLearningVelocity(data: UserPerformanceData[]): number {
    ...
    const accuracies = data.map(d => (d.correctAnswers / d.totalQuestions) * 100);
    let totalChange = 0;
  ...
  private calculateEngagementScore(data: UserPerformanceData[]): number {
    ...
    const avgStudyTime = data.reduce((sum, d) => sum + d.totalStudyTime, 0) / data.length;
    const avgStreak = data.reduce((sum, d) => sum + d.streakDays, 0) / data.length;
    const avgAccuracy = data.reduce((sum, d) => sum + (d.correctAnswers / d.totalQuestions) * 100, 0) / data.length;
    
    // Weighted engagement score
    return Math.min(100, (avgStudyTime * 0.3) + (avgStreak * 5) + (avgAccuracy * 0.4));
  }
  ...
  private getEmptyMetrics(userId: number, timeframe: AnalyticsTimeframe): LearningMetrics {
    return {
      userId,
      timeframe,
      totalStudyTime: 0,
  ...
  private generateProgressChart(data: UserPerformanceData[]): ChartData {
    ...
    return {
      ...
      datasets: [{
        label: 'Nøjagtighed (%)',
        data: sortedData.map(d => (d.correctAnswers / d.totalQuestions) * 100),
        borderColor: '#3B82F6',
  ...
  private generateAccuracyTrendChart(data: UserPerformanceData[]): ChartData {
    ...
    return {
      ...
      datasets: [{
        ...
        data: last30Days.map(d => (d.correctAnswers / d.totalQuestions) * 100),
        borderColor: '#10B981',
  ...
  private generateConceptMasteryRadar(conceptMastery: ConceptMasteryData[]): ChartData {
  ...
  private generateDifficultyProgressionChart(progression: DifficultyProgressionData[]): ChartData {
    return {
  ...
  private calculateMasteryLevel(accuracy: number, timeSpent: number, sessions: number): number {
  ...
  private async generateConceptRecommendations(
    ...
    if (masteryLevel < 50) {
      recommendations.push(`Fokuser mere på ${conceptName} - øv grundlæggende principper`);
    }
    
    if (accuracy < 70) {
      recommendations.push(`Gennemgå teori for ${conceptName} før flere øvelser`);
    }
    
    if (trend === 'declining') {
      recommendations.push(`${conceptName} viser nedadgående trend - genbesøg materialet`);
    }
    
    if (masteryLevel > 80) {
      recommendations.push(`Godt arbejde med ${conceptName}! Prøv mere avancerede opgaver`);
    }
  ...
  private generateStudyScheduleRecommendation(
    ...
    const avgAccuracy = data.reduce((sum, d) => 
      sum + (d.correctAnswers / d.totalQuestions) * 100, 0) / data.length;
  ...
  private async getAIProgressPrediction(
    ...
    const daysToCompletion = trajectory === 'excellent' ? 30 :
                            trajectory === 'good' ? 45 :
                            trajectory === 'average' ? 60 : 90;
    ...
    return {
      completionDate,
      confidence: trajectory === 'excellent' ? 0.9 : 
                 trajectory === 'good' ? 0.8 :
                 trajectory === 'average' ? 0.6 : 0.4,
      riskFactors: metrics.accuracyRate < 70 ? ['Lav nøjagtighed', 'Inkonsistent øvelse'] : [],
      opportunities: ['Øg studietid', 'Fokuser på svage områder', 'Brug spaced repetition'],
      milestones: [
        {
          milestoneId: 'intermediate',
          milestoneName: 'Intermediate niveau',
          predictedDate: new Date(Date.now() + (daysToCompletion * 0.6 * 24 * 60 * 60 * 1000)),
          confidence: 0.8,
