...
export class SearchCacheService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.defaultTTL = this.configService.get<number>('SEARCH_CACHE_TTL', 5 * 60 * 1000);
    // Maximum number of cached entries (default: 1000)
    this.maxCacheSize = this.configService.get<number>('SEARCH_CACHE_MAX_SIZE', 1000);
  ...
  private generateCacheKey(params: any): string {
    const normalizedParams = {
      ...
      status: Array.isArray(params.status) ? params.status.sort() : params.status,
    };
  ...
  set(params: any, data: any, ttl?: number): void {
    ...
    if (this.cache.size >= this.maxCacheSize) {
      ...
      this.logger.debug(`Cache size limit reached, removed oldest entry: ${oldestKey}`);
    }
  ...
  invalidateByEntity(entityType: string, entityId?: string): void {
    ...
    if (keysToDelete.length > 0) {
      this.logger.debug(`Invalidated ${keysToDelete.length} cache entries for entity type: ${entityType}`);
    }
  }
