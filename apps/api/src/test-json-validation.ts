...
function testJsonValidation() {
  ...
  try {
    ...
    console.log(
      'Invalid social links passed validation (should fail):',
      result,
    );
  } catch (error) {
    if (error instanceof ZodError) {
      console.log(
        'Invalid social links correctly failed validation:',
  ...
  try {
    ...
    console.log(
      'Invalid user settings passed validation (should fail):',
      result,
    );
  } catch (error) {
    if (error instanceof ZodError) {
      console.log(
        'Invalid user settings correctly failed validation:',
        error.errors.map((e) => e.message).join(', '),
      );
    } else {
      console.error('Unknown error:', error);
