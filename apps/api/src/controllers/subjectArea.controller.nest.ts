...
export class SubjectAreaController {
  ...
  async getAllSubjectAreas(): Promise<SubjectAreaDto[]> {
    try {
      return await this.prisma.subjectArea.findMany({
        orderBy: { name: 'asc' },
      });
    } catch (error) {
  ...
  async createSubjectArea(
    ...
    try {
      ...
      const existingSubjectArea = await this.prisma.subjectArea.findUnique({
        where: { slug },
      });
      ...
      return await this.prisma.subjectArea.create({
        data: {
          name,
          slug,
        },
  ...
  async updateSubjectArea(
    ...
    try {
      ...
      if (slug && slug !== existingSubjectArea.slug) {
        const slugExists = await this.prisma.subjectArea.findUnique({
          where: { slug },
        });
      ...
      return await this.prisma.subjectArea.update({
        where: { id },
        data: {
  ...
  @ApiOperation({ summary: 'Slet et fagområde' })
  @ApiParam({ name: 'id', description: 'Fagområde ID', type: Number })
  @ApiResponse({
    status: 200,
  ...
  async deleteSubjectArea(
    ...
    try {
      ...
      await this.prisma.subjectArea.delete({
        where: { id },
      });
