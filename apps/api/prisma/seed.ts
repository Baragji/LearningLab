...
function parseSeedpensum(content: string): Record<string, string[]> {
  ...
  for (const line of lines) {
    ...
    // Check if line is a topic header
      ...
      trimmedLine.match(/^[A-Za-zæøåÆØÅ\s]+$/) &&
  ...
  try {
    ...
    const createdCourses = await Promise.all(
      semesterKeys.map(async (semesterKey, index) => {
        ...
        const semesterNumber = semesterNumberMatch
          ? parseInt(semesterNumberMatch[1])
          : index + 1;
    ...
    console.log(
      'Created education programs, courses, topics, lessons, content blocks, and quizzes.',
    );
  } catch (error) {
