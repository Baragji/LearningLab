...
const SearchPage = () => {
  ...
  const [searchType, setSearchType] = useState<'course' | 'module' | 'lesson' | 'all'>(initialType);
  ...
  const [subjectAreaId, setSubjectAreaId] = useState<number | undefined>(initialSubjectAreaId);
  ...
  const [subjectAreas, setSubjectAreas] = useState<{ id: number; name: string }[]>([]);
  const [isLoadingSubjectAreas, setIsLoadingSubjectAreas] = useState(true);
  
  // State for active tab
  const [activeTab, setActiveTab] = useState<'all' | 'courses' | 'modules' | 'lessons'>(
    initialType === 'all' ? 'all' : 
    initialType === 'course' ? 'courses' : 
    initialType === 'module' ? 'modules' : 'lessons'
  );
  ...
  useEffect(() => {
    const fetchSubjectAreas = async () => {
      ...
      try {
        ...
        setSubjectAreas(response.data.subjectAreas.map((area: any) => ({
          id: area.id,
          name: area.name
        })));
      } catch (error) {
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select 
                  value={difficulty || ''} 
                  onValueChange={(value) => setDifficulty(value as Difficulty || undefined)}
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value={Difficulty.BEGINNER}>Begynder</SelectItem>
                    <SelectItem value={Difficulty.INTERMEDIATE}>Øvet</SelectItem>
                    <SelectItem value={Difficulty.ADVANCED}>Avanceret</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select 
                  ...
                  onValueChange={(value) => setSubjectAreaId(value ? parseInt(value) : undefined)}
        ...
        <div className="space-y-6">
          ...
          <Tabs value={activeTab} onValueChange={handleTabChange}>
            <TabsList className="mb-6">
              ...
              <TabsTrigger value="courses" className="flex items-center gap-2">
                ...
                {results.courses.length > 0 && (
                  <Badge variant="secondary" className="ml-1">
                    {searchType === 'course' ? results.total : results.courses.length}
                  </Badge>
                )}
              </TabsTrigger>
              <TabsTrigger value="modules" className="flex items-center gap-2">
                ...
                {results.modules.length > 0 && (
                  <Badge variant="secondary" className="ml-1">
                    {searchType === 'module' ? results.total : results.modules.length}
                  </Badge>
                )}
              </TabsTrigger>
              <TabsTrigger value="lessons" className="flex items-center gap-2">
                ...
                {results.lessons.length > 0 && (
                  <Badge variant="secondary" className="ml-1">
                    {searchType === 'lesson' ? results.total : results.lessons.length}
                  </Badge>
                )}
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="all">
              {results.courses.length === 0 && results.modules.length === 0 && results.lessons.length === 0 ? (
                  ...
                  {results.courses.length > 0 && (
                    <div>
                      <div className="flex justify-between items-center mb-4">
                        ...
                        {results.courses.length < (searchType === 'course' ? results.total : 3) && (
                          <Button 
                            variant="link" 
                            onClick={() => {
                  ...
                  {results.modules.length > 0 && (
                    <div>
                      <div className="flex justify-between items-center mb-4">
                        ...
                        {results.modules.length < (searchType === 'module' ? results.total : 3) && (
                          <Button 
                            variant="link" 
                            onClick={() => {
                          ...
                          <Card key={module.id}>
                            ...
                            <CardFooter className="pt-2">
                              <Button 
                                variant="outline" 
                                size="sm"
                                onClick={() => router.push(`/courses/${module.course.slug}/modules/${module.id}`)}
                              >
                                Se modul
                              </Button>
                            </CardFooter>
                  ...
                  {results.lessons.length > 0 && (
                    <div>
                      <div className="flex justify-between items-center mb-4">
                        ...
                        {results.lessons.length < (searchType === 'lesson' ? results.total : 3) && (
                          <Button 
                            variant="link" 
                            onClick={() => {
                        ...
                        {results.lessons.map((lesson) => (
                          <Card key={lesson.id}>
                            <CardHeader className="pb-2">
                              ...
                              <CardDescription>
                                Fra kurset: {lesson.module.course.title} / Modul: {lesson.module.title}
                              </CardDescription>
                            </CardHeader>
                            <CardContent className="pb-2">
                              <p className="line-clamp-2 text-sm">
                            ...
                            <CardFooter className="pt-2">
                              <Button 
                                variant="outline" 
                                size="sm"
                                onClick={() => router.push(`/courses/${lesson.module.course.slug}/modules/${lesson.module.id}/lessons/${lesson.id}`)}
                              >
                                Se lektion
                              </Button>
                            </CardFooter>
                  ...
                  {results.totalPages > 1 && (
                    <div className="flex justify-between items-center mt-8">
                      <div className="text-sm text-muted-foreground">
                        Viser {(results.page - 1) * results.limit + 1}-{Math.min(results.page * results.limit, results.total)} af {results.total} kurser
                      </div>
              ...
              {results.modules.length === 0 ? (
                ...
                <>
                  ...
                  {results.totalPages > 1 && (
                    <div className="flex justify-between items-center mt-8">
                      <div className="text-sm text-muted-foreground">
                        Viser {(results.page - 1) * results.limit + 1}-{Math.min(results.page * results.limit, results.total)} af {results.total} moduler
                      </div>
                      <div className="flex gap-2">
                        <Button 
                          variant="outline" 
              ...
              {results.lessons.length === 0 ? (
                ...
                <>
                  ...
                  {results.totalPages > 1 && (
                    <div className="flex justify-between items-center mt-8">
                      <div className="text-sm text-muted-foreground">
                        Viser {(results.page - 1) * results.limit + 1}-{Math.min(results.page * results.limit, results.total)} af {results.total} lektioner
                      </div>
                      <div className="flex gap-2">
                        <Button 
                          variant="outline" 
