...
import { Loader2, ArrowLeft, Save, Trash2, UserPlus, Users, UserMinus } from 'lucide-react';
...
const GroupEditPage = ({ params }: { params: { id: string } }) => {
    ...
    try {
      ...
      setFormData({
        ...
        permissions: {
          ...
          canManageCourses: response.data.permissions?.canManageCourses || false,
          canManageContent: response.data.permissions?.canManageContent || false,
  ...
  const fetchGroupMembers = async (page = 1) => {
    ...
    try {
      const response = await apiClient.get(`/api/user-groups/${groupId}/users?page=${page}&limit=10`);
  ...
  const fetchAvailableUsers = async () => {
    ...
    try {
      const response = await apiClient.get(`/api/user-groups/${groupId}/available-users`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiClient.put(`/api/user-groups/${groupId}`, updateData);
      setGroupData(response.data);
  ...
  const handleRemoveUserFromGroup = async () => {
    ...
    try {
      await apiClient.delete(`/api/user-groups/${groupId}/users/${selectedUserId}`);
    ...
    <div className="container mx-auto py-8">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-3xl font-bold">
          {isLoadingGroup ? 'Indlæser brugergruppe...' : `Rediger brugergruppe: ${groupData?.name}`}
        </h1>
      </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {groupData?.permissions ? (
                        Object.entries(groupData.permissions).map(([key, value]) => (
                          <div key={key} className="flex items-center">
                            <div className={`w-4 h-4 rounded-full mr-2 ${value ? 'bg-primary' : 'bg-muted'}`} />
                            <span>
                        ...
                        <p className="text-muted-foreground">Ingen tilladelser defineret</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="mb-3">{new Date(groupData?.createdAt || '').toLocaleDateString('da-DK')}</p>
                    
                    <p className="text-sm text-muted-foreground mb-1">Sidst opdateret</p>
                    <p>{new Date(groupData?.updatedAt || '').toLocaleDateString('da-DK')}</p>
                  </div>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center space-x-2">
                          <Checkbox 
                            ...
                            onCheckedChange={(checked) => 
                              handlePermissionChange('canManageUsers', !!checked)
                            }
                          />
                        ...
                        <div className="flex items-center space-x-2">
                          <Checkbox 
                            ...
                            onCheckedChange={(checked) => 
                              handlePermissionChange('canManageCourses', !!checked)
                            }
                          />
                        ...
                        <div className="flex items-center space-x-2">
                          <Checkbox 
                            ...
                            onCheckedChange={(checked) => 
                              handlePermissionChange('canManageContent', !!checked)
                            }
                          />
                        ...
                        <div className="flex items-center space-x-2">
                          <Checkbox 
                            ...
                            onCheckedChange={(checked) => 
                              handlePermissionChange('canViewReports', !!checked)
                            }
                          />
                      ...
                      {totalPages > 1 && (
                        <div className="flex justify-between items-center mt-4">
                          <div className="text-sm text-muted-foreground">
                            Viser {(currentPage - 1) * 10 + 1}-{Math.min(currentPage * 10, totalMembers)} af {totalMembers} brugere
                          </div>
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="py-4">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Denne handling kan ikke fortrydes. Brugere i gruppen vil miste de tilladelser, der er tildelt gennem denne gruppe.
            </p>
          </div>
          <DialogFooter>
                    ...
                    <TableHeader>
                      <TableRow>
                        <TableHead className="w-12">
                          <Checkbox 
                            checked={selectedUserIds.length > 0 && selectedUserIds.length === availableUsers.length}
                            onCheckedChange={(checked) => {
                              if (checked) {
      ...
      <Dialog open={isRemoveUserDialogOpen} onOpenChange={setIsRemoveUserDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="py-4">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Brugeren vil miste de tilladelser, der er tildelt gennem denne gruppe.
            </p>
          </div>
          <DialogFooter>
