...
test.describe('AI Features E2E Tests', () => {
  ...
  test('should generate questions from content', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="generated-question"]').first()).toBeVisible();
  });

  test('should test AI question generation API endpoints', async ({ page }) => {
    // Test the API endpoints directly
    const response = await page.request.post('http://localhost:5002/api/ai/questions/generate-advanced', {
      data: {
  ...
  test('should test content embedding API', async ({ page }) => {
    const response = await page.request.post('http://localhost:5002/api/ai/embeddings/create', {
      data: {
  ...
  test('should test content search functionality', async ({ page }) => {
    ...
    // Test search
    const searchResponse = await page.request.post('http://localhost:5002/api/search', {
      data: {
        query: 'JavaScript',
  ...
  test('should test AI usage logging', async ({ page }) => {
    ...
    await page.request.post('http://localhost:5002/api/ai/questions/generate-advanced', {
      data: {
    ...
    const logsResponse = await page.request.get('http://localhost:5002/api/ai/usage-logs');
  ...
  test('should handle AI service errors gracefully', async ({ page }) => {
    ...
    await expect(page.locator('text=Indtast venligst noget indhold')).toBeVisible();
  });

  test('should test question quality scoring', async ({ page }) => {
    const response = await page.request.post('http://localhost:5002/api/ai/questions/generate-advanced', {
      data: {
        content: 'Detailed content about machine learning algorithms and their applications in data science',
        contentType: 'lesson',
        contentId: 'ml-lesson',
  ...
  test('should test content processing pipeline', async ({ page }) => {
    ...
    const response = await page.request.post('http://localhost:5002/api/ai/content/process', {
      data: {
        content: 'Complex educational content that needs to be processed and analyzed',
        contentType: 'lesson',
    ...
    const statusResponse = await page.request.get(`http://localhost:5002/api/ai/content/process/${data.processingId}/status`);
  ...
  test('should test AI configuration management', async ({ page }) => {
    ...
    const configResponse = await page.request.get('http://localhost:5002/api/ai/config');
    ...
    const updateResponse = await page.request.put('http://localhost:5002/api/ai/config', {
      data: {
        maxTokens: 1000,
        temperature: 0.7,
        model: 'gpt-3.5-turbo'
...
test.describe('Ollama Local AI Integration', () => {
  ...
  test('should use Ollama for question generation if available', async ({ page }) => {
    // Check if Ollama is available
    try {
      const ollamaCheck = await page.request.get('http://localhost:11434/api/tags');
...
test.describe('AI Performance and Load Testing', () => {
  test('should handle multiple concurrent AI requests', async ({ page }) => {
    ...
    for (let i = 0; i < 5; i++) {
      requests.push(
        page.request.post('http://localhost:5002/api/ai/questions/generate-advanced', {
          data: {
