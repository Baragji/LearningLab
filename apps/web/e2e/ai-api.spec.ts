...
test.describe('AI API Endpoints Testing', () => {
  ...
  test('should test AI question generation endpoint', async ({ request }) => {
    const response = await request.post(`${API_BASE_URL}/ai/questions/generate-advanced`, {
      data: {
        content: 'JavaScript er et programmeringssprog. Variabler kan deklareres med let, const eller var.',
        contentType: 'lesson',
    ...
    data.questions.forEach((question: any) => {
      ...
      expect(['MULTIPLE_CHOICE', 'FILL_IN_BLANK'].includes(question.type)).toBe(true);
  ...
  test('should test lesson-based question generation', async ({ request }) => {
    ...
    if (lessonResponse.status() === 201) {
      ...
      const response = await request.post(`${API_BASE_URL}/ai/questions/generate/lesson/${lessonId}`, {
        data: {
          numberOfQuestions: 2,
  ...
  test('should test topic-based question generation', async ({ request }) => {
    ...
    const response = await request.post(`${API_BASE_URL}/ai/questions/generate/topic/1`, {
      data: {
        numberOfQuestions: 2,
  ...
  test('should test content embedding creation', async ({ request }) => {
    const response = await request.post(`${API_BASE_URL}/ai/embeddings/create`, {
      data: {
        content: 'Dette er test indhold for embedding generering. Det handler om JavaScript programmering.',
        contentType: 'lesson',
  ...
  test('should test content embedding search', async ({ request }) => {
    ...
    await request.post(`${API_BASE_URL}/ai/embeddings/create`, {
      data: {
        content: 'JavaScript arrays er en datastruktur til at gemme flere værdier.',
        contentType: 'lesson',
    ...
    // Test search
    const response = await request.post(`${API_BASE_URL}/ai/embeddings/search`, {
      data: {
  ...
  test('should test content processing pipeline', async ({ request }) => {
    const response = await request.post(`${API_BASE_URL}/ai/content/process`, {
      data: {
        content: 'Langt indhold der skal processeres og analyseres af AI systemet for at skabe embeddings og metadata.',
        contentType: 'lesson',
    ...
    const statusResponse = await request.get(`${API_BASE_URL}/ai/content/process/${data.processingId}/status`);
  ...
  test('should test error handling for invalid requests', async ({ request }) => {
    // Test with missing required fields
    const response1 = await request.post(`${API_BASE_URL}/ai/questions/generate-advanced`, {
      data: {
        // Missing content
        contentType: 'lesson',
        targetDifficulty: 'INTERMEDIATE'
    ...
    const response2 = await request.post(`${API_BASE_URL}/ai/questions/generate-advanced`, {
      data: {
    ...
    const response3 = await request.post(`${API_BASE_URL}/ai/questions/generate-advanced`, {
      data: {
        content: 'Test content',
        contentType: 'lesson',
        targetDifficulty: 'INTERMEDIATE',
  ...
  test('should test question bank integration', async ({ request }) => {
    ...
    const response = await request.post(`${API_BASE_URL}/ai/questions/generate-to-bank`, {
      data: {
