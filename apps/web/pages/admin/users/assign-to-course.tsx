  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const ids = (userIds as string).split(',').map(id => parseInt(id, 10));
        
        // Fetch user details
        const usersPromise = axios.post<User[]>('/api/users/bulk-get', { userIds: ids });
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Tildel brugere til kursus</h1>
          <button
            onClick={() => router.back()}
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-md max-h-60 overflow-y-auto">
                    {selectedUsers.length === 0 ? (
                      <p className="text-gray-500 dark:text-gray-400">Ingen brugere valgt</p>
                    ) : (
                ...
                <div className="flex justify-end space-x-3">
                  ...
                  <button
                    type="submit"
                    disabled={submitting || !selectedCourseId || selectedUsers.length === 0 || courses.length === 0}
                    className={`px-4 py-2 rounded-md ${
                      submitting || !selectedCourseId || selectedUsers.length === 0 || courses.length === 0
