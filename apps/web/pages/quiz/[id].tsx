...
const QuizPage: React.FC = () => {
  ...
  const [startQuizAttempt, { isLoading: isStartingAttempt }] = useStartQuizAttemptMutation();
  const [submitAnswer, { isLoading: isSubmittingAnswer }] = useSubmitAnswerMutation();
  const [completeQuizAttempt, { isLoading: isCompletingAttempt }] = useCompleteQuizAttemptMutation();
  ...
  const handleNextQuestion = async () => {
    ...
    if (currentQuestion && quizState.answers[currentQuestion.id] && quizState.attemptId) {
  ...
  const handleSubmitQuiz = async () => {
    ...
    const allQuestionsAnswered = quiz.questions.every(
      question => quizState.answers[question.id] && quizState.answers[question.id].length > 0
    );
          ...
          <div className="flex justify-between items-center">
            ...
            {!quizState.isCompleted && (
              <div className="text-sm text-gray-500 dark:text-gray-400">
                Spørgsmål {quizState.currentQuestionIndex + 1} af {quiz.questions.length}
              </div>
            )}
          </div>
        ...
        {quizState.isCompleted && quizState.result && (
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <div className="text-center mb-6">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Quiz Gennemført!</h2>
              <p className="text-gray-600 dark:text-gray-300">
                Du har besvaret {quizState.result.correctAnswers} ud af {quizState.result.totalQuestions} spørgsmål korrekt.
              </p>
            </div>
            ...
            <div className="flex justify-center mb-8">
              <div className="w-48 h-48 relative">
                <svg viewBox="0 0 36 36" className="w-full h-full">
                  ...
                  />
                  <text x="18" y="20.5" className="text-5xl font-bold fill-gray-900 dark:fill-white text-center" textAnchor="middle">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg text-center">
                <p className="text-sm text-blue-700 dark:text-blue-300 mb-1">Score</p>
                <p className="text-2xl font-bold text-blue-800 dark:text-blue-200">{quizState.result.score}%</p>
              </div>
              
              <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg text-center">
                <p className="text-sm text-green-700 dark:text-green-300 mb-1">XP Optjent</p>
                <p className="text-2xl font-bold text-green-800 dark:text-green-200">{quizState.result.xpEarned} XP</p>
              </div>
              
              <div className="bg-purple-50 dark:bg-purple-900/20 p-4 rounded-lg text-center">
                <p className="text-sm text-purple-700 dark:text-purple-300 mb-1">Korrekte Svar</p>
            ...
            {quizState.result.feedback && (
              <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg mb-6">
                ...
                <p className="text-gray-700 dark:text-gray-300">{quizState.result.feedback}</p>
              </div>
            )}
            ...
            <div className="flex justify-between">
              ...
              {quizState.currentQuestionIndex < quiz.questions.length - 1 ? (
                <button
                  ...
                  disabled={!quizState.answers[currentQuestion.id] || isSubmittingAnswer}
