...
const QuizQuestion: React.FC<QuizQuestionProps> = ({
  ...
  const getOptionStatus = (option: AnswerOption) => {
    if (showCorrectAnswer) {
      ...
      if (selectedAnswerId === option.id && !option.isCorrect) return 'incorrect';
      return 'neutral';
    }
          ...
          {answerOptions.map((option, index) => {
            ...
            return (
              <Fade in={true} timeout={300 + index * 100} key={option.id}>
                <Card
                  sx={optionStyles}
                  onMouseEnter={() => !isReviewMode && setHoveredOption(option.id)}
                    ...
                    <FormControlLabel
                      ...
                      control={
                        <Radio 
                          ...
                          sx={{
                            color: status === 'correct' ? theme.palette.success.main :
                                   status === 'incorrect' ? theme.palette.error.main :
                                   status === 'selected' ? theme.palette.primary.main :
                      ...
                      label={
                        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                          <Typography 
                            variant="body1" 
                            sx={{ 
                              fontWeight: selectedAnswerId === option.id ? 600 : 400,
                              color: status === 'correct' ? theme.palette.success.dark :
                                     status === 'incorrect' ? theme.palette.error.dark :
                          ...
                          {showCorrectAnswer && (
                            <Box>
                              ...
                              {selectedAnswerId === option.id && !option.isCorrect && (
                                <Chip 
                                  label="Your Answer" 
                                  size="small" 
                                  color="error" 
      ...
      {question.explanation && (showCorrectAnswer || isReviewMode) && (
        <Fade in={true} timeout={500}>
          <Box sx={{ 
            ...
            <Typography variant="subtitle1" color="info.dark" gutterBottom sx={{ fontWeight: 600 }}>
              💡 Explanation
            </Typography>
