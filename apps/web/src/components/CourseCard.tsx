...
export const CourseCard: React.FC<CourseCardProps> = ({
  ...
  const handleUnenroll = async () => {
    if (
      window.confirm(
        "Er du sikker på, at du vil afmelde dig fra dette kursus? Al din fremgang vil blive slettet.",
      )
    ) {
      try {
  ...
  const totalItems =
    (course as any).topics?.reduce(
      (total: number, topic: any) =>
        total + (topic.lessons?.length || 0) + (topic.quizzes?.length || 0),
      0,
    ) || 0;
      ...
      <div className="h-48 bg-gradient-to-r from-blue-500 to-purple-600 flex items-center justify-center">
        <div className="text-white text-center">
          ...
          <p className="text-sm opacity-90">
            {(course as any).educationProgram?.name}
          </p>
        </div>
      </div>
