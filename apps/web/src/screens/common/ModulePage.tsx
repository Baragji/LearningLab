...
const ModulePage: React.FC = () => {
  ...
  const { data: userProgress, isLoading: isProgressLoading } =
    useGetUserProgressQuery(course?.id || 0, { skip: !course?.id });

  const isLoading =
    isModuleLoading || isLessonsLoading || isCourseLoading || isProgressLoading;
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 8 }}>
      ...
      >
        {subjectArea && (
          <Link href={`/subject-area/${subjectArea.id}`} passHref>
            <Typography
              component="a"
