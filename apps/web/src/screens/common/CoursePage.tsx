...
const CoursePage: React.FC = () => {
  ...
  const [enrollInCourse, { isLoading: isEnrolling }] =
    useEnrollInCourseMutation();
  const [unenrollFromCourse, { isLoading: isUnenrolling }] =
    useUnenrollFromCourseMutation();
  ...
  const handleUnenroll = async () => {
    if (
      window.confirm(
        "Er du sikker på, at du vil framelde dig dette kursus? Al din fremgang vil gå tabt.",
      )
    ) {
      try {
      ...
      <Paper elevation={2} sx={{ p: 4, mb: 4 }}>
        ...
        <Box sx={{ mt: 3, mb: 3 }}>
          {enrollmentStatus?.enrolled ? (
            ...
            >
              <Chip label="Ikke tilmeldt" color="default" variant="outlined" />
              <Button
              ...
              <Grid item xs={12} sm={6} md={4} key={module.id}>
                <Card
                  ...
                  sx={{
                    ...
                    border: !enrollmentStatus?.enrolled
                      ? "2px dashed #ccc"
                      : "none",
                  }}
                >
                  <CardActionArea
                    ...
                    onClick={() =>
                      isUnlocked && router.push(`/module/${module.id}`)
                    }
                    disabled={!isUnlocked}
                  >
