...
interface AuthContextType {
  ...
  signup: (name: string | undefined, email: string, password: string) => Promise<void>; // Function to create a new user
  forgotPassword: (email: string) => Promise<string>; // Function to request password reset
  resetPassword: (token: string, newPassword: string, confirmPassword: string) => Promise<string>; // Function to reset password
...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const fetchUserProfile = async (currentToken: string) => {
    ...
    try {
      ...
      console.log('AuthContext: Fetching user profile from:', `${baseUrl}/auth/profile`);
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          console.error('AuthContext: Invalid token when fetching profile. Logging out.');
        ...
        throw new Error(`Could not fetch user profile: ${response.status} ${response.statusText}`);
      }
  ...
  const refreshAccessToken = async () => {
    ...
    try {
      setIsLoading(true);
      const baseUrl = process.env.NEXT_PUBLIC_API_URL || '/api';
      console.log('AuthContext: Attempting to renew token with API URL:', baseUrl);
      ...
      if (!response.ok) {
        throw new Error(`Could not renew token: ${response.status} ${response.statusText}`);
      }
  ...
  useEffect(() => {
    ...
    if (storedToken) {
      console.log('AuthContext: Token found in localStorage, attempting to fetch profile.');
      ...
      if (Date.now() >= expiresAt && storedRefreshToken) {
        console.log('AuthContext: Token is expired, attempting to renew with refresh token.');
  ...
  useEffect(() => {
    ...
    const refreshTimeout = setTimeout(() => {
      console.log('AuthContext: Scheduling token renewal 5 minutes before expiry.');
      refreshAccessToken();
  ...
  const resetPassword = async (token: string, newPassword: string, confirmPassword: string) => {
    ...
    try {
      ...
      return data.message || 'Password has been reset successfully. You can now log in with your new password.';
    } catch (error: any) {
